...
<div class="p-6 bg-white rounded-lg shadow-md">
    ...
    @foreach($kepalauptd as $item)
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">

        {{-- Foto Profil --}}
        <div class="flex flex-col items-center border border-emerald-500 p-4 rounded-lg shadow">
            <img src="{{ $item->foto_profil ? asset('storage/' . $item->foto_profil) : 'https://via.placeholder.com/150' }}"
                alt="Foto Profil Kepala UPTD" class="w-40 h-40 rounded-full object-cover shadow mb-4">
            <h2 class="text-xl font-semibold text-center text-gray-800">{{ $item->user->username ?? '-' }}</h2>
        ...
        <div class="md:col-span-2 bg-white border border-emerald-500 p-6 rounded-lg shadow space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-medium text-gray-800">{{ $item->nip ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Email</p>
                    <p class="font-medium text-gray-800">{{ $item->user->email ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Nomor HP</p>
                    <p class="font-medium text-gray-800">{{ $item->user->phone ?? '-' }}</p>
                </div>
                <div>
            ...
            {{-- Tombol Edit --}}
            <div class="pt-4">
                <a href="{{ route('dashboard.kepalauptd.edit', $item->id) }}"
