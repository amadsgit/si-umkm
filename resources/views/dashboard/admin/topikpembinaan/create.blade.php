...
@section('title', 'Tambah Topik Pembinaan')

@section('content')
<div class="max-w-3xl mx-auto px-6">
    <div class="bg-white shadow-xl rounded-3xl p-10 border border-gray-200">
        <h2 class="text-xl font-bold mb-10 text-left text-emerald-500">Tambah Topik Pembinaan</h2>

        <form action="{{ route('admin.topik-pembinaan.store') }}" method="POST" class="grid grid-cols-1 gap-6">
            @csrf

            {{-- Nama Pembinaan --}}
            <div>
                <label class="block mb-2 text-sm font-semibold text-gray-700">Nama Topik Pembinaan</label>
                <input type="text" name="nama_topik_pembinaan" value="{{ old('nama_topik_pembinaan') }}"
