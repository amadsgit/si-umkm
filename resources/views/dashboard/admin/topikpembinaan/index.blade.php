...
@section('title', 'Data Topik Pembinaan')
...
<div class="p-6 bg-white rounded-xl shadow-lg space-y-6">
    ...
    <div class="bg-white p-6 rounded-xl shadow">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-semibold text-emerald-700">Daftar Topik Pembinaan</h2>
            <a href="{{ route('admin.topik-pembinaan.create') }}"
                ...
                <i class="ph ph-plus-circle text-lg mr-2"></i> Tambah Topik Pembinaan
            </a>
        </div>
        ...
        <table class="min-w-full table-auto border border-gray-200">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">Nama Topik Pembinaan</th>
            ...
            <tbody>
                @forelse($topikPembinaanList as $item)
                <tr class="border-t border-gray-200">
                    <td class="px-4 py-2 text-sm text-gray-800">{{ $loop->iteration }}</td>
                    <td class="px-4 py-2 text-sm text-gray-800">{{ $item->nama_topik_pembinaan }}</td>
                    ...
                    <td class="px-4 py-2 text-sm text-gray-800">
                        <div class="flex items-center gap-2">
                            {{-- Edit --}}
                            <a href="{{ route('admin.topik-pembinaan.edit', $item->id) }}"
                            ...
                            {{-- Hapus --}}
                            <form action="{{ route('admin.topik-pembinaan.destroy', $item->id) }}" method="POST"
                                onsubmit="return confirm('Yakin ingin menghapus topik pembinaan ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <tr>
                    <td colspan="5" class="px-4 py-4 text-sm text-gray-500 text-center">
                        Belum ada data topik pembinaan.
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="mt-4 mb-4">
            {{ $topikPembinaanList->links('vendor.pagination.tailwind') }}
        </div>
    </div>
</div>
@endsection