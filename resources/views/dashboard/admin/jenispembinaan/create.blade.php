...
@section('title', 'Tambah Jenis Pembinaan')

@section('content')
<div class="max-w-3xl mx-auto px-6">
    <div class="bg-white shadow-xl rounded-3xl p-10 border border-gray-200">
        <h2 class="text-xl font-bold mb-10 text-left text-emerald-500">Tambah Jenis Pembinaan</h2>

        <form action="{{ route('admin.jenis-pembinaan.store') }}" method="POST" class="grid grid-cols-1 gap-6">
            @csrf

            {{-- Nama Pembinaan --}}
            <div>
                <label class="block mb-2 text-sm font-semibold text-gray-700">Nama Jenis Pembinaan</label>
                <input type="text" name="nama_pembinaan" value="{{ old('nama_pembinaan') }}"
